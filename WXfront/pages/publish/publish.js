...
Page({
  ...
  data: {
    ...
    state:"publish",
    tabList:[
      {name:'发布商品信息',index:'publish'},
      {name:'发布求购信息',index:'need'}
    ],
    refreshState: false,
    isSelect:false,//展示类型？
...
getType:function(e){
  ...
  submit1: function (e) {
    ...
    wx.request({
      url: 'https://www.cyhhyt.xyz/idle/demands', //接口名称,传入图片数组参数   
      header: {
        'content-type': 'application/json' // 默认值（固定，我开发过程中还没有遇到需要修改header的）   
      },
      method: 'POST', //请求方式    
      data: JSON.stringify({
        ...
        described: this.data.inputValue5,
      }), //用于存放post请求的参数     
      success(res) {
        console.log(res.data) //成功之后的回调   
      },  
